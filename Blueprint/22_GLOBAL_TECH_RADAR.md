...
- **2026-09-26 二次查證更正**：先前誤判「N1X 是規格較低的變體」——**N1X 其實是 NVIDIA 對 RTX Spark 這顆晶片本身的代號**，不是獨立的低規格款。**Surface Laptop Ultra（ULTRA-MAERA-2）與 Surface RTX Spark Dev Box（SPARK-AGAVE-3/4）用的是同一顆晶片**（20核 Grace CPU + Blackwell GPU、最高128GB統一記憶體、1 petaFLOP），差別只在**外型與散熱能力**：Dev Box 桌機造型、100W TDP，可承受長時間高負載訓練/agentic pipeline/本地微調；Laptop Ultra 受筆電散熱與電池限制，無法長時間全速輸出。也就是說 ULTRA-MAERA-2 跟 SPARK-AGAVE-3/4 **理論上算力等級接近**，主要差在「能不能長時間全速跑」，這點原本盤點沒抓到，值得排進 [Blueprint/03](03_MACHINE_ARCHITECTURE.md) 的機器角色設計考量。Stephen 另提供的 biggo 報導確認 SPARK-AGAVE-3/4 是**兩台規格相同的 Dev Box**（各自獨立，不是同一台機器內兩顆晶片），與 SuperBrain 藍圖「FAST/DEEP 各一台」的設計一致。
